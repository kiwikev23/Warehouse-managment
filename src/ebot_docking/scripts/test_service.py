...
class MyRobotDockingController(Node):
    ...
    def controller_loop(self,goal,bool):
        ...
        if not self.is_aligned: 
            ...
            kp = 0.5
            # vel = Twist()
            vel.angular.z = kp*error_z
            self.vel_pub.publish(vel)

            if np.abs(error_z)<0.03:
                print("what")
                vel.angular.z = 0.0 
        ...
        elif self.is_aligned and self.is_docking and bool==True:
            # error = 0
            # if self.usrleft_value>=0.2:
            print("hey?")
            vel = Twist()  
            error_x = self.usrleft_value - 0.06
            kp = 0.5
        ...
        elif self.is_aligned and self.is_docking and bool==False:
            # error = 0
            # if self.usrleft_value>=0.2:
            ...
            vel = Twist()  
            error_x = (-3.155 - self.robot_pose[1])
            kp = 0.7
            vel.linear.x =  - kp * error_x
            # print("\n lin vel: ",vel.linear.x)
